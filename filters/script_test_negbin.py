...
results. The NegBin Model for incoming traffic
"""
...
for d in range(days):
    ...
    params, W = icf.pf_update_negbin(y_test[0, d], x, params, W)

    for t in range(1, T):
        if t % 10 == 0:
            print("Filtering - day "+str(d)+" step "+str(t))
        predicted[t, d] = icf.predict_mean(x, params, W)
        x, params, W = icf.pf_step_negbin(y_test[t, d], x, params, W)
...
## Save results
np.savez("results_negbin.npz", rmse=rmse, predicted=predicted)
