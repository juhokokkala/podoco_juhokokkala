...
results. Poisson Model 1 for incoming traffic.
"""
...
invlscale_local_mcmc = mcmc_data[:, mcmc_header.index('invlscale_local')]
s_local_mcmc = mcmc_data[:, mcmc_header.index('s_local')]

params0['sqrtQ_x'] = np.sqrt(1 - np.exp(-2 * dt * invlscale_local_mcmc)) \
                      * s_local_mcmc

params0['A_x'] = np.exp(-dt * invlscale_local_mcmc)
...
np.savez("results_poisson1.npz", rmse=rmse, predicted=predicted)
