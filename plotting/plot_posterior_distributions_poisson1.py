...
###############################################################################
"""Plot MCMC results, Poisson Model 1 for incoming traffic."""
...
## Incoming -- Poisson Model 1

print("Reading samples from Stan output")
stanfile = "../stan_models/output_ic_poisson.csv"
...
lscale_local_mcmc = 1/mcmc_data[:, mcmc_header.index('invlscale_local')]
s_local_mcmc = mcmc_data[:, mcmc_header.index('s_local')]

lscale_base_mcmc = 1/mcmc_data[:, mcmc_header.index('invlscale_base')]
...
tikz_save('../report/fig/ic_poisson1_mcmc_predmean.tikz',
...
tikz_save('../report/fig/ic_poisson1_mcmc_base.tikz',
          figureheight='\\figureheight',
          figurewidth='\\figurewidth')


##
def plot_param_hist(sample, name, prior, bins=30, xlim=None, filename=None):
    plt.cla()
...
plot_param_hist(lscale_local_mcmc, '$\\rho_\mathrm{(l)}$',
                lambda x: (1 / x**2) * np.exp(-1/x),
                filename='../report/fig/ic_poisson1_mcmc_lscale_local.tikz')

plot_param_hist(lscale_base_mcmc, '$\\rho_\mathrm{(b)}$',
                lambda x: (0.25 / x**2) * np.exp(-0.25/x),
                filename='../report/fig/ic_poisson1_mcmc_lscale_base.tikz')

plot_param_hist(s_local_mcmc, '$\sigma_\mathrm{(l)}$',
                lambda x: 2 * stats.t.pdf(x, 2),
                filename='../report/fig/ic_poisson1_mcmc_s_local.tikz')

plot_param_hist(s_base_mcmc, '$\sigma_\mathrm{(b)}$',
                lambda x: 2 * stats.t.pdf(x, 2),
                filename='../report/fig/ic_poisson1_mcmc_s_base.tikz')
