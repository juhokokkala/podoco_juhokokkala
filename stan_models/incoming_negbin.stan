/*
 * NegBin model for incoming traffic
...
parameters {
    real mu;                         //Long-term mean of the baseline
    real<lower=0> lscale_base;       //Lengthscale of the baseline
    real<lower=0> s_base;            //Standard deviation of the baseline
    real<lower=0> lscale_local;      //Lengthscale of the local variation
    real<lower=0> s_local;           //St. dev. of the local variation
    vector[N] x_base_shock;          //Iid N(0,1) transformed to the baseline
    vector[N] x_shock[D];            //Iid N(0,1) transformed to intensities
    real<lower=0, upper=1> invomega; //Inverse of dispersion parameter 
}
...
transformed parameters {
    ...
    Q[2] = s_base * s_base * (lda * lda - exp(-2 * dt * lda) * (lda * lda * 
            ...
            lda));
    covQ = 2 * s_base * s_base * dt * dt * lda * lda * lda * exp(-2 * dt *             lda);
    ...
    A_local = exp(-dt / lscale_local);
    sqrtQ_local = s_local * sqrt(1 - exp(-2 * dt / lscale_local));
...
model {
    //MODEL SPECIFICATION
    //Parameter priors
    lscale_local    ~ student_t(2, 0, 1); 
    s_local         ~ student_t(2, 0, 1); 
    lscale_base     ~ student_t(2, 0, 0.25); 
    ...
    //Observation model
    for (d in 1:D) {
       y_ic[d]      ~ neg_binomial_2_log(x[d],exp(x[d])/(1/invomega-1));
    }
